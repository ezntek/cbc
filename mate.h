...
#define ALLOC_ATTR2(sz, al)                                                    \
    __attribute__((malloc, alloc_size(sz), alloc_align(al), returns_nonnull))
...
#else
#error                                                                         \
    "The codebase only supports linux, macos, FreeBSD, windows, android and emscripten"
#endif
#endif
...
#include <windows.h>
#if !defined(                                                                  \
    ENABLE_VIRTUAL_TERMINAL_PROCESSING) // old SDKs sometimes dont have it
...
int32_t __base_vec_partition(void** data, size_t element_size,
                             CompareFunc compare, int32_t low, int32_t high);
void __base_vec_quicksort(void** data, size_t element_size, CompareFunc compare,
                          int32_t low, int32_t high);

#define VecSort(vector, compare)                                               \
    __base_vec_quicksort((void**)&(vector).data, sizeof(*(vector).data),       \
                         compare, 0, (vector).length - 1)
...
#define VecPush(vector, value)                                                 \
    __base_vec_push((void**)&(vector).data, &(vector).length,                  \
                    &(vector).capacity, sizeof(*(vector).data), &(value));
void __base_vec_push(void** data, size_t* length, size_t* capacity,
                     size_t element_size, void* value);

#define VecPop(vector)                                                         \
    __base_vec_pop((vector).data, &(vector).length, sizeof(*(vector).data));
void* __base_vec_pop(void* data, size_t* length, size_t element_size);

#define VecShift(vector)                                                       \
    __base_vec_shift((void**)&(vector).data, &(vector).length,                 \
                     sizeof(*(vector).data))
void __base_vec_shift(void** data, size_t* length, size_t element_size);

#define VecUnshift(vector, value)                                              \
    __base_vec_unshift((void**)&(vector).data, &(vector).length,               \
                       &(vector).capacity, sizeof(*(vector).data), &(value))
void __base_vec_unshift(void** data, size_t* length, size_t* capacity,
                        size_t element_size, const void* value);

#define VecInsert(vector, value, index)                                        \
    __base_vec_insert((void**)&(vector).data, &(vector).length,                \
                      &(vector).capacity, sizeof(*(vector).data), &(value),    \
                      (index))
void __base_vec_insert(void** data, size_t* length, size_t* capacity,
                       size_t element_size, void* value, size_t index);

#define VecAt(vector, index)                                                   \
    (*(__typeof__(*(vector).data)*)__base_vec_at((void**)&(vector).data,       \
                                                 &(vector).length, index,      \
                                                 sizeof(*(vector).data)))
void* __base_vec_at(void** data, size_t* length, size_t index,
                    size_t elementSize);

#define VecAtPtr(vector, index)                                                \
    (__base_vec_at((void**)&(vector).data, &(vector).length, (index),          \
                   sizeof(*(vector).data)))

#define VecFree(vector)                                                        \
    __base_vec_free((void**)&(vector).data, &(vector).length,                  \
                    &(vector).capacity)
void __base_vec_free(void** data, size_t* length, size_t* capacity);

#define VecForEach(vector, it)                                                 \
    for (__typeof__(*(vector).data)*(it) = (vector).data;                      \
         (vector).data && (it) < (vector).data + (vector).length; (it)++)
...
#define Assert(expression, ...)                                                \
    (void)((!!(expression)) ||                                                 \
           (_custom_assert(#expression, __FILE__, __LINE__, __VA_ARGS__), 0))
static void _custom_assert(const char* expr, const char* file, unsigned line,
                           const char* format, ...) FORMAT_CHECK(4, 5);

#define Unreachable(...)                                                       \
    (void)((_custom_unreachable(__FILE__, __LINE__, __VA_ARGS__), 0))
static void _custom_unreachable(const char* file, unsigned line,
                                const char* format, ...) FORMAT_CHECK(3, 4);
...
#define STRING_LENGTH(s)                                                       \
    ((sizeof((s)) / sizeof((s)[0])) -                                          \
     sizeof((s)[0])) // NOTE: Inspired from clay.h
#define ENSURE_STRING_LITERAL(x) ("" x "")

// NOTE: If an error led you here, it's because `S` can only be used with string
// literals, i.e. `S("SomeString")` and not `S(yourString)` - for that use `s()`
#define S(string)                                                              \
    (TYPE_INIT(String){.length = STRING_LENGTH(ENSURE_STRING_LITERAL(string)), \
                       .data = (char*)(uintptr_t)(string)})
String s(char* msg);
...
String StrNewSize(Arena* arena, char* str,
                  size_t len); // len without null terminator
...
WARN_UNUSED FileReadResult FileRead(Arena* arena, String path,
                                    size_t file_size);
...
[[maybe_unused]] static inline void __df_cb(__df_t* __fp) {
    (*__fp)();
}
...
void __base_vec_quicksort(void** data, size_t element_size, CompareFunc compare,
    ...
    if (low < high) {
        int32_t pi =
            __base_vec_partition(data, element_size, compare, low, high);
...
void __base_vec_push(void** data, size_t* length, size_t* capacity,
    ...
    Assert(*length <= *capacity,
           "VecPush: Possible memory corruption or vector not initialized, "
           "`Vector vector = {0}`");
    Assert(!(*length > 0 && *data == NULL),
           "VecPush: Possible memory corruption, data should be NULL only if "
           "length == 0");
...
void __base_vec_shift(void** data, size_t* length, size_t element_size) {
    ...
    memmove(*data, (char*)(*data) + element_size,
            ((*length) - 1) * element_size);
    (*length)--;
}
...
void __base_vec_insert(void** data, size_t* length, size_t* capacity,
    ...
    if (index < *length) {
        memmove((char*)(*data) + ((index + 1) * element_size),
...
#if !defined(PLATFORM_WIN)
WARN_UNUSED errno_t memcpy_s(void* dest, size_t destSize, const void* src,
                             size_t count);
...
void* Malloc(size_t size) {
    ...
    Assert(address != NULL,
           "Malloc: failed, returned address should never be NULL");
    return address;
}
...
void* Realloc(void* block, size_t size) {
    ...
    Assert(address != NULL,
           "Realloc: failed, returned address should never be NULL");
    return address;
}
...
String F(Arena* arena, const char* format, ...) {
    ...
    va_start(args, format);
    size_t size =
        vsnprintf(NULL, 0, format, args) + 1; // +1 for null terminator
    va_end(args);
...
String StrNew(Arena* arena, char* str) {
    ...
    size_t memory_size =
        sizeof(char) * len + 1; // NOTE: Includes null terminator
...
void StrCopy(String* destination, String source) {
    Assert(!StrIsNull(*destination),
           "StrCopy: destination should never be NULL");
    Assert(!StrIsNull(source), "StrCopy: source should never be NULL");
    Assert(destination->length >= source.length,
           "destination length should never smaller than source length");

    errno_t err = memcpy_s(destination->data, destination->length + 1,
...
String StrConcat(Arena* arena, String string1, String string2) {
    if (StrIsNull(string1)) {
        ...
        errno_t err = memcpy_s(allocated_string, memory_size, string2.data,
                               string2.length);
    ...
    if (StrIsNull(string2)) {
        ...
        size_t memory_size = sizeof(char) * len + 1;
        char* allocated_string = ArenaAllocChars(arena, memory_size);
        errno_t err = memcpy_s(allocated_string, memory_size, string1.data,
                               string1.length);
        Assert(err == SUCCESS, "StrConcat: memcpy_s failed, err: %d", err);
    ...
    errno_t err =
        memcpy_s(allocated_string, memory_size, string1.data, string1.length);
    Assert(err == SUCCESS, "StrConcat: memcpy_s failed, err: %d", err);

    err = memcpy_s(allocated_string + string1.length, memory_size, string2.data,
                   string2.length);
    Assert(err == SUCCESS, "StrConcat: memcpy_s failed, err: %d", err);
...
String StrSlice(Arena* arena, String str, size_t start, ssize_t end) {
    ...
    Assert(end >= (ssize_t)start,
           "StrSlice: end must be greater than or equal to start: start=%lu, "
           "end=%lu",
           start, end);
...
bool StrIncludes(String source, String sub_str) {
    ...
    if (source.length == 0 || sub_str.length == 0 ||
        sub_str.length > source.length) {
        return false;
    }
...
void SBAdd(StringBuilder* builder, String string) {
    ...
    memcpy(builder->buffer.data + builder->buffer.length, string.data,
           string.length);
...
void SBAddFormatV(StringBuilder* builder, char* fmt, va_list args) {
    ...
    while (*cursor) {
        if (*cursor != '%') {
            ...
            SBAdd(builder, (String){.data = (char*)start,
        ...
        switch (*cursor++) {
            ...
            default: Unreachable("SBAddFormatV: unknown format specifier");
        }
...
int32_t RandomInteger(int32_t min, int32_t max) {
    Assert(min <= max,
           "RandomInteger: min should always be less than or equal to max");
...
FileReadResult FileRead(Arena* arena, String path, size_t file_size) {
    ...
    HANDLE hFile = CreateFileA(path.data, GENERIC_READ, FILE_SHARE_READ, NULL,
...
Error FileWrite(String path, String data) {
    HANDLE hFile = CreateFileA(path.data, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS,
    ...
    DWORD bytes_written;
    Error err = SUCCESS;
    if (!WriteFile(hFile, data.data, (DWORD)data.length, &bytes_written, NULL))
        err = ErrnoMatch(GetLastError());
    else if ((size_t)bytes_written != data.length)
...
Error FileAdd(String path, String data) {
    HANDLE hFile = CreateFileA(path.data, GENERIC_WRITE, 0, NULL, OPEN_ALWAYS,
                               FILE_ATTRIBUTE_NORMAL, NULL);
    if (hFile == INVALID_HANDLE_VALUE)
        return ErrnoMatch(GetLastError());
...
Error FileRename(String old_path, String new_path) {
    if (!MoveFileExA(old_path.data, new_path.data, MOVEFILE_REPLACE_EXISTING))
        return ErrnoMatch(GetLastError());
    return SUCCESS;
}
...
ListDirResult ListDir(Arena* arena, String path) {
    ...
    if (err != ERROR_NO_MORE_FILES)
        result.error = ErrnoMatch(err);
    FindClose(hFind);
    return result;
}

Error FileCopy(String src_path, String dest_path) {
    HANDLE src = CreateFileA(src_path.data, GENERIC_READ, FILE_SHARE_READ, NULL,
                             OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    HANDLE dest = CreateFileA(dest_path.data, GENERIC_WRITE, 0, NULL,
    ...
    Error err = SUCCESS;
    while (ReadFile(src, buffer, sizeof(buffer), &bytes_read, NULL) &&
           bytes_read > 0) {
    ...
    if (err == SUCCESS) {
        ...
        if (loop_err != ERROR_SUCCESS && loop_err != ERROR_HANDLE_EOF &&
            loop_err != ERROR_ALREADY_EXISTS && loop_err != open_err) {
...
GetCwdResult GetCwd(void) {
    ...
    result.data =
        (String){.data = curr_path, .length = strnlen(curr_path, PATH_MAX)};
    return result;
}
...
Error FileDelete(String path) {
    if (unlink(path.data) != SUCCESS)
        return ErrnoMatch(errno);
    return SUCCESS;
}

Error FileRename(String old_path, String new_path) {
    if (rename(old_path.data, new_path.data) != SUCCESS)
        return ErrnoMatch(errno);
    return SUCCESS;
}

Error Mkdir(String path) {
    if (mkdir(path.data, 0755) == SUCCESS)
        return SUCCESS;

    if (errno == EEXIST) {
        struct stat st;
        if (stat(path.data, &st) == SUCCESS && S_ISDIR(st.st_mode))
            return SUCCESS;
    }
    return ErrnoMatch(errno);
}
...
Error FileCopy(String sourcePath, String destPath) {
    ...
    dest_fd = open(destPath.data, O_WRONLY | O_CREAT | O_TRUNC,
...
IniParseResult IniParse(String path) {
    ...
    for (size_t i = 0; i < buffer.length; i++) {
        ...
        if (curr_char == '\n' || i == buffer.length - 1) {
            ...
            newline_pos = i;
            {
                String key = {.data = buffer.data + start_pos,
                              .length = equal_pos - start_pos};
                key.data[key.length] = '\0';

                String value = {.data = (buffer.data) + (equal_pos + 1),
                                .length = newline_pos - (equal_pos + 1)};
...
int32_t IniGetInt(IniFile* ini_file, String key) {
    ...
    if (end_ptr == value.data) {
        LogWarn("IniGetLong: Failed to convert [key: %s, value: %s] to int",
                key.data, value.data);
        return 0;
    }
...
int64_t IniGetLong(IniFile* ini_file, String key) {
    ...
    if (end_ptr == value.data) {
        LogWarn("IniGetLong: Failed to convert [key: %s, value: %s] to long",
                key.data, value.data);
        return 0;
    }
...
float64_t IniGetDouble(IniFile* ini_file, String key) {
    ...
    if (end_ptr == value.data) {
        LogWarn("IniGetLong: Failed to convert [key: %s, value: %s] to double",
                key.data, value.data);
        return 0.0;
    }
...
#define CreateCompileCommands(target)                                          \
    mate_create_compile_commands((target).ninjaBuildPath);
static WARN_UNUSED CreateCompileCommandsError
mate_create_compile_commands(String ninjaBuildPath);
...
static void mate_add_library_paths(String* targetLibs, char** libs,
                                   size_t libs_size);
...
static void mate_link_system_libraries(String* targetLibs, char** libs,
                                       size_t libs_size);
...
static void mate_link_frameworks(String* targetLibs, char** frameworks,
                                 size_t frameworks_size);

#define LinkFrameworksWithOptions(target, options, ...)                        \
    do {                                                                       \
        char* _frameworks[] = {__VA_ARGS__};                                   \
        mate_link_frameworks_with_options(&(target).libs, options,             \
                                          _frameworks, ARR_LEN(_frameworks));  \
    } while (0)
static void mate_link_frameworks_with_options(String* targetLibs,
                                              LinkFrameworkOptions options,
                                              char** frameworks,
                                              size_t frameworks_size);
...
static void mate_add_include_paths(String* targetIncludes, char** includes,
...
static void mate_add_framework_paths(String* targetIncludes, char** includes,
                                     size_t includes_size);
...
#define RemoveFile(target, source)                                             \
    mate_remove_file(&(target).sources, s(source));
...
#define FlagBuilderAdd(builder, ...)                                           \
    mate_flag_builder_add_list(builder, (char*[]){__VA_ARGS__, NULL})
...
/* --- Path Utils --- */
static String mate_path_with_platform_ext(Arena* arena, String path,
                                          String unix_ext, String win_ext);
...
#define RunCommandF(format, ...)                                               \
    RunCommand(F(mate_state.arena, format, __VA_ARGS__))
...
static void mate_set_default_state(void) {
    {
        ...
        Assert(cwd_result.error == SUCCESS,
               "mate_set_default_state: failed at getting current working "
               "directory, with error %s",
               ErrToStr(cwd_result.error).data);
...
void CreateConfig(MateOptions options) {
    ...
    if (options.compiler != 0)
        mate_state.compiler = options.compiler;
    if (options.mateExe != NULL)
        mate_state.mate_exe = AbsoluteNormPathExe(s(options.mateExe));
    if (options.mateSource != NULL)
        mate_state.mate_source = AbsoluteNormPath(s(options.mateSource));
    if (options.buildDirectory != NULL)
        mate_state.build_directory =
            AbsoluteNormPath(s(options.buildDirectory));
}

static void mate_read_cache(void) {
    String mate_cache_path =
        PathJoin(mate_state.build_directory, S("mate-cache.ini"));

    IniParseResult parse_result = IniParse(mate_cache_path);
    Assert(parse_result.error == SUCCESS,
           "MateReadCache: failed reading MateCache at %s, err: %s",
           mate_cache_path.data, ErrToStr(parse_result.error).data);
    mate_state.cache = parse_result.data;

    mate_state.mate_cache.last_build =
        IniGetLong(&mate_state.cache, S("modify-time"));
    if (mate_state.mate_cache.last_build == 0) {
        mate_state.mate_cache.first_build = true;
        mate_state.mate_cache.last_build = TimeNow() / 1000;

        String modify_time =
            F(mate_state.arena, FMT_I64, mate_state.mate_cache.last_build);
    ...
    if (mate_state.mate_cache.first_build) {
        ...
        Assert(ninjaCheck == SUCCESS,
               "MateReadCache: Ninja build system not found. Please install "
               "Ninja and add it to your PATH.");
    }
#else
    mate_state.mate_cache.samurai_build =
        IniGetBool(&mate_state.cache, S("samurai-build"));
    if (mate_state.mate_cache.samurai_build == false) {
        Assert(mate_state.mate_cache.first_build,
               "MateCache: This is not the first build and samurai is not "
               "compiled, could be a cache error, delete `./build` folder and "
               "rebuild `./mate.c`");

        String samurai_amalgam = s(SAMURAI_AMALGAM);
        String source_path =
            PathJoin(mate_state.build_directory, S("samurai.c"));
        Error err_file_write = FileWrite(source_path, samurai_amalgam);
        Assert(err_file_write == SUCCESS,
               "MateReadCache: failed writing samurai source code to path %s",
               source_path.data);

        String output_path = PathJoin(mate_state.build_directory, S("samurai"));
        String compile_command =
            F(mate_state.arena, "%s \"%s\" -o \"%s\" -std=c99",
              CompilerToStr(mate_state.compiler).data, source_path.data,
              output_path.data);

        errno_t run_error = RunCommand(compile_command);
        Assert(run_error == SUCCESS,
               "MateReadCache: Error meanwhile compiling samurai at %s, if you "
               "are seeing this please make an issue at "
               "github.com/TomasBorquez/mate.h",
               source_path.data);
    ...
    Assert(write_error == SUCCESS,
           "MateReadCache: Failed writing cache, err: %d", write_error);
}

static bool mate_need_rebuild(void) {
    FileStatsResult stats_result = FileStats(mate_state.mate_source);
    Assert(stats_result.error == SUCCESS,
           "could not read fileStats for %s, error: %d",
           mate_state.mate_source.data, stats_result.error);
    ...
    Assert(write_error == SUCCESS,
           "could not write cache for path %s, error: %d", mate_cache_path.data,
           write_error);
...
static void mate_rebuild(void) {
    ...
    String mate_exe_new =
        NormPathExe(PathJoin(mate_state.build_directory, S("mate-new")));
    String mate_exe_old =
        NormPathExe(PathJoin(mate_state.build_directory, S("mate-old")));

    String compile_command;
    if (isMSVC()) {
        compile_command = F(mate_state.arena, "cl.exe \"%s\" /Fe:\"%s\"",
                            mate_state.mate_source.data, mate_exe_new.data);
    } else {
        compile_command = F(mate_state.arena, "%s \"%s\" -o \"%s\"",
                            CompilerToStr(mate_state.compiler).data,
                            mate_state.mate_source.data, mate_exe_new.data);
    }

    LogWarn("%s changed rebuilding...", mate_state.mate_source.data);
    errno_t rebuild_error = RunCommand(compile_command);
    Assert(rebuild_error == SUCCESS, "MateRebuild: failed command %s, err: %d",
           compile_command.data, rebuild_error);

    errno_t rename_error = FileRename(mate_exe, mate_exe_old);
    Assert(rename_error == SUCCESS,
           "MateRebuild: failed renaming original executable failed, err: %d",
           rename_error);

    rename_error = FileRename(mate_exe_new, mate_exe);
    Assert(rename_error == SUCCESS,
           "MateRebuild: failed renaming new executable into old: %d",
           rename_error);
...
void StartBuild(void) {
    ...
    Assert(Mkdir(mate_state.build_directory) == SUCCESS,
           "StartBuild: mkdir failed on making path %s",
           mate_state.build_directory.data);
    mate_read_cache();
    mate_rebuild();
}

static void mate_apply_warning_flags(FlagBuilder* fb, Compiler c,
                                     FlagWarnings w) {
...
static void mate_apply_optimization_flags(FlagBuilder* fb, Compiler c,
                                          FlagOptimization o) {
    static char* general[][2] = {
...
static void mate_apply_sanitizer_flags(FlagBuilder* fb, Compiler c,
                                       FlagSanitizer s) {
    static char* general[][4] = {
        [FLAG_SANITIZER_ADDRESS] = {"fsanitize=address",
                                    "fno-omit-frame-pointer", NULL},
...
static void mate_apply_error_flags(FlagBuilder* fb, Compiler c,
                                   FlagErrorFormat e) {
    static char* general[][5] = {
        [FLAG_ERROR] = {"fdiagnostics-color=always", NULL},
        [FLAG_ERROR_MAX] = {"fdiagnostics-color=always",
                        "fdiagnostics-show-caret", "fdiagnostics-show-option",
                        "fdiagnostics-generate-patch", NULL},
    };
    static char* clang[][5] = {
        [FLAG_ERROR] = {"fcolor-diagnostics", NULL},
        [FLAG_ERROR_MAX] = {"fcolor-diagnostics", "fcaret-diagnostics",
                        "fdiagnostics-fixit-info", "fdiagnostics-show-option", NULL},
    };
    static char* msvc[][3] = {
...
Executable CreateExecutable(ExecutableOptions opts) {
    Assert(mate_state.init_config,
           "CreateExecutable: before creating an executable you must use "
           "StartBuild(), like this: \n"
           "\n"
           "StartBuild()\n"
           "{\n"
           " // ...\n"
    ...
    Assert(opts.output != NULL,
           "CreateExecutable: failed, ExecutableOptions.output should never be "
           "null, please define the output name like this: \n"
           "\n"
           "CreateExecutable((ExecutableOptions) { .output = \"main\"});");
    Assert(
        mate_is_valid_output(s(opts.output)),
    ...
    if (opts.flags != NULL)
        SBAdd(&fb, s(opts.flags));
    if (opts.warnings != 0)
        mate_apply_warning_flags(&fb, mate_state.compiler, opts.warnings);
    if (opts.debug != 0)
        mate_apply_debug_flags(&fb, mate_state.compiler, opts.debug);
    if (opts.optimization != 0)
        mate_apply_optimization_flags(&fb, mate_state.compiler,
                                      opts.optimization);
    if (opts.std != 0)
        mate_apply_std_flags(&fb, mate_state.compiler, opts.std);
    if (opts.sanitizer != 0)
        mate_apply_sanitizer_flags(&fb, mate_state.compiler, opts.sanitizer);

    mate_apply_error_flags(&fb, mate_state.compiler, opts.error);
    if (!StrIsNull(fb.buffer))
        result.flags = fb.buffer;

    if (opts.libs != NULL)
        result.libs = s(opts.libs);
    if (opts.includes != NULL)
        result.includes = s(opts.includes);
    if (opts.linkerFlags != NULL)
        result.linkerFlags = s(opts.linkerFlags);

    String build_file_name =
        F(mate_state.arena, "exe-%s.ninja", PathStem(result.output).data);
    result.ninjaBuildPath =
        PathJoin(mate_state.build_directory, build_file_name);
    return result;
}

StaticLib CreateStaticLib(StaticLibOptions opts) {
    Assert(!isMSVC(), "CreateStaticLib: MSVC compiler not yet implemented for "
                      "static libraries");
    Assert(mate_state.init_config,
           "CreateStaticLib: before creating a static library you must use "
           "StartBuild(), like this: \n"
           "\n"
           "StartBuild();\n"
           "{\n"
           ...
           "EndBuild();");
    Assert(opts.output != NULL,
           "CreateStaticLib: failed, StaticLibOptions.output should never be "
           "null, please define the output name like this: \n"
           "\n"
    ...
    if (opts.includes != NULL)
        result.includes = s(opts.includes);
    if (opts.arFlags != NULL)
        result.arFlags = s(opts.arFlags);

    String build_file_name =
        F(mate_state.arena, "static-%s.ninja", PathStem(result.output).data);
    result.ninjaBuildPath =
        PathJoin(mate_state.build_directory, build_file_name);
    return result;
}

static CreateCompileCommandsError
mate_create_compile_commands(String ninjaBuildPath) {
    FILE* ninja_pipe;
    char buffer[4096];
    size_t bytes_read;
    ...
    if (mate_state.mate_cache.samurai_build == true) {
        String samurai_output_path =
            PathJoin(mate_state.build_directory, S("samurai"));
        compdb_cmd = NormPath(F(mate_state.arena, "%s -f %s -t compdb compile",
                                samurai_output_path.data, ninjaBuildPath.data));
    }

    if (mate_state.mate_cache.samurai_build == false) {
        compdb_cmd =
            NormPath(F(mate_state.arena, "ninja -f %s -t compdb compile",
                       ninjaBuildPath.data));
    }

    String compile_commands_path = NormPath(
        PathJoin(mate_state.build_directory, S("compile_commands.json")));

    FILE* output_file = NULL;
#if defined(PLATFORM_WIN)
    errno_t err = fopen_s(&output_file, compile_commands_path.data, "w");
    Assert(err == SUCCESS, "CreateCompileCommands: failed to fopen_s path %s",
           compile_commands_path.data);
#else
    output_file = fopen(compile_commands_path.data, "w");
    Assert(output_file != NULL,
           "CreateCompileCommands: failed to open_s path %s, error: %s",
           compile_commands_path.data, strerror(errno));
#endif

    ninja_pipe = popen(compdb_cmd.data, "r");
    if (ninja_pipe == NULL) {
        LogError("CreateCompileCommands: Failed to run compdb command, %s",
                 compdb_cmd.data);
        fclose(output_file);
    ...
    LogSuccess("Successfully created %s",
               NormPathEnd(compile_commands_path).data);
...
static void mate_add_file(StringVector* sources, String source) {
    ...
    Assert(source.data[source.length - 1] != '/',
           "AddFile: failed to add a file, you can't add a slash at the end of "
           "a path.\n"
           "For example, valid: AddFile(\"./main.c\"), invalid: "
           "AddFile(\"./main.c/\")");
    ...
    String pattern =
        StrSlice(mate_state.arena, source, last_slash + 1, source.length);

    ListDirResult list_dir_result = ListDir(mate_state.arena, directory);
    Error error = list_dir_result.error;
    Assert(list_dir_result.error == SUCCESS,
           "AddFile: failed at getting files in directory %s, error %s",
           directory.data, ErrToStr(error).data);
...
static bool mate_remove_file(StringVector* sources, String source) {
    Assert(sources->length > 0, "RemoveFile: Before removing a file you must "
                                "first add a file, use: AddFile()");
...
static void mate_install_executable(Executable* executable) {
    Assert(executable->sources.length != 0,
           "InstallExecutable: Executable has zero sources, add at least one "
           "with AddFile(\"./main.c\")");
    Assert(!StrIsNull(executable->output),
           "InstallExecutable: Before installing executable you must first "
           "CreateExecutable()");
    ...
    { // Link command
        SBAddS(&builder, "rule link\n"
                         "  command = $cc");
        if (executable->flags.length > 0)
            SBAddS(&builder, " $flags");
        if (executable->linkerFlags.length > 0)
            SBAddS(&builder, " $linker_flags");
    ...
    Assert(write_error == SUCCESS,
           "InstallExecutable: failed to write build.ninja for %s, err: %d",
           ninja_build_path.data, write_error);

    String build_command;
    if (mate_state.mate_cache.samurai_build) {
        String samurai_output_path =
            PathJoin(mate_state.build_directory, S("samurai"));
        build_command = F(mate_state.arena, "%s -f %s",
                          samurai_output_path.data, ninja_build_path.data);
    } else {
        build_command =
            F(mate_state.arena, "ninja -f %s", ninja_build_path.data);
    }

    errno_t run_error = RunCommand(build_command);
    Assert(
        run_error == SUCCESS,
        "InstallExecutable: Ninja file compilation failed with code: " FMT_I32,
        run_error);

    mate_state.total_time = TimeNow() - mate_state.start_time;
    executable->outputPath =
        PathJoin(mate_state.build_directory, executable->output);

    VecFree(executable->sources);
}

static void mate_install_static_lib(StaticLib* static_lib) {
    Assert(static_lib->sources.length != 0,
           "InstallStaticLib: Static Library has zero sources, add at least "
           "one with AddFile(\"./main.c\")");
    Assert(!StrIsNull(static_lib->output),
           "InstallStaticLib: Before installing static library you must first "
           "CreateStaticLib()");
    ...
    { // Build individual source files
        ...
        for (size_t i = 0; i < static_lib->sources.length; i++) {
            ...
            if (mate_state.compiler == GCC || mate_state.compiler == CLANG) {
                String depFile = StrNewSize(mate_state.arena, output_file.data,
                                            output_file.length);
    ...
    Error write_error = FileWrite(ninja_build_path, builder.buffer);
    Assert(write_error == SUCCESS,
           "InstallStaticLib: failed to write build-static-library.ninja for "
           "%s, err: %d",
           ninja_build_path.data, write_error);
    ...
    Assert(
        run_error == SUCCESS,
        "InstallStaticLib: Ninja file compilation failed with code: " FMT_I32,
        run_error);

    mate_state.total_time = TimeNow() - mate_state.start_time;
    static_lib->outputPath =
        PathJoin(mate_state.build_directory, static_lib->output);
...
static void mate_link_frameworks_with_options(String* targetLibs,
                                              ...
                                              size_t frameworks_size) {
    Assert(isClang(),
           "LinkFrameworks: This function is only supported for Clang.");

    char* framework_flag = NULL;
    switch (options) {
        case NONE: framework_flag = "-framework"; break;
        case NEEDED: framework_flag = "-needed_framework"; break;
        case WEAK: framework_flag = "-weak_framework"; break;
        default:
            Unreachable(
                "LinkFrameworks: Invalid framework linking option provided.");
    }
...
static void mate_add_include_paths(String* target_includes, char** includes,
                                   size_t includes_size) {
    StringBuilder builder = SBCreate(mate_state.arena);
...
static void mate_add_framework_paths(String* target_includes, char** frameworks,
                                     size_t frameworks_size) {
    Assert(isClang() || isGCC(),
           "AddFrameworkPaths: This function is only supported for GCC/Clang.");
...
static void mate_flag_builder_add_string(FlagBuilder* builder, char* flag) {
    ...
    if (mate_state.compiler == MSVC) {
        Assert(flag[0] != '/',
               "FlagBuilderAdd: flag should not contain '/'. Your flag:\n%s "
               "\n\ne.g usage FlagBuilderAdd(\"W4\")",
               flag);
        SBAdd(builder, is_empty ? S("/") : S(" /"));
    } else {
        Assert(flag[0] != '-',
               "FlagBuilderAdd: flag should not contain '-'. Your flag:\n%s "
               "\n\ne.g usage FlagBuilderAdd(\"Wall\")",
               flag);
        SBAdd(builder, is_empty ? S("-") : S(" -"));
    }
...
static String mate_path_get_ext(String path) {
    ...
    if (last_dot <= filename_start)
        return S("");
    return (String){.data = path.data + last_dot,
                    .length = path.length - last_dot};
}
...
static String mate_path_with_platform_ext(Arena* arena, String path,
    ...
    result =
        target_ext.length > 0
            ? StrConcat(arena, StrNewSize(arena, result.data, result.length),
                        target_ext)
...
String PathStem(String path) {
    ...
    for (size_t i = 0; i < no_ext.length; i++) {
        if (no_ext.data[i] == '/' || no_ext.data[i] == '\\')
            filename_start = i + 1;
    }

    String result = StrNewSize(mate_state.arena, no_ext.data + filename_start,
                               no_ext.length - filename_start);
    return mate_path_fix_slashes(result);
}

String NormPath(String path) {
    String stripped = mate_path_strip_dot_slash(path);
    return mate_path_fix_slashes(
        StrNewSize(mate_state.arena, stripped.data, stripped.length));
}
...
String NormPathEnd(String path) {
    ...
    return StrNewSize(mate_state.arena, path.data + last_slash,
                      path.length - last_slash);
}
...
String NormPathStaticLib(String str) {
    return mate_path_with_platform_ext(mate_state.arena, str, S(".a"),
                                       S(".lib"));
}
...
String NormPathOutput(String str) {
    ...
    Assert(stem.length > 0, "NormPathOutput: failed to get stem from %s",
           str.data);
...
String CompilerToStr(Compiler compiler) {
    switch (compiler) {
        ...
        default:
            Unreachable("CompilerToStr: failed, should never get here, "
                        "compiler given does not exist: %d",
                        compiler);
            return S("");
    }
}
